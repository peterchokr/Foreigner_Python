...
### Example 3: Guestbook (예제 3: 방문록)

This is a program that allows visitors to sign a guestbook and view all previous entries.

이것은 방문자가 방문록에 서명하고 이전의 모든 방문 기록을 볼 수 있는 프로그램입니다.

```python
# Guestbook program (방문록 프로그램)
print("📝" + "=" * 38 + "📝")
print("   Guestbook")
print("📝" + "=" * 38 + "📝")
...
### Example 5: Game Settings (예제 5: 게임 설정)

This is a program that saves and loads game settings in JSON format. Users can adjust various settings and save them for the next session.

이것은 게임 설정을 JSON 형식으로 저장하고 불러오는 프로그램입니다. 사용자가 다양한 설정을 조정하고 다음 세션을 위해 저장할 수 있습니다.

```python
# Game settings management program (게임 설정 관리 프로그램)
import json
import os
...
def save_config(config):
    """Save game configuration (게임 설정 저장)"""
...
while True:
    ...
    print("\n1. Adjust sound")
    print("2. Adjust music")
    print("3. Change display mode")
    ...
    if choice == "1":
        volume = int(input("Sound (0-100): "))
        if 0 <= volume <= 100:
            config["sound"] = volume
            print(f"✓ Sound set to {volume}")
        else:
            print("Enter value between 0-100.")
  
    elif choice == "2":
        music = int(input("Music (0-100): "))
        if 0 <= music <= 100:
            config["music"] = music
            print(f"✓ Music set to {music}")
        else:
            print("Enter value between 0-100.")
        ...
        if mode == "1":
            ...
            config["display"] = "Windowed"
  
        print(f"✓ Display mode: {config['display']}")
        ...
        confirm = input("Reset to default settings? (Y/N): ")
        if confirm == "Y" or confirm == "y":
...
CSV files store data separated by commas and can be parsed with `split(",")`. JSON uses a dictionary-like format and is handled with `json.dump()` and `json.load()`.
...
Which code separates the CSV line "Alex Johnson,20,Computer Science" into a list?

CSV 파일의 한 줄 "Alex Johnson,20,Computer Science"를 리스트로 분리하는 코드는?
...
CSV uses commas as separators, so `split(",")` separates the data into ["Alex Johnson", "20", "Computer Science"].

CSV는 쉼표로 구분되므로 `split(",")`을 사용하여 ["Alex Johnson", "20", "Computer Science"]로 분리합니다.
