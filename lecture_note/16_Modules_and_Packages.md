...
### Example 3: Utility Module (예제 3: 유틸리티 모듈)
...
```python
# utils.py - Collection of utility functions (유틸리티 함수 모음)
"""
Utility function collection (유틸리티 함수 모음)
"""

def print_line(char="=", length=40):
    """Print a separator line (구분선 출력)"""
    print(char * length)

def print_title(title, char="=", length=40):
    """Print a centered title (제목 출력)"""
    print(char * length)
    print(title.center(length))
    print(char * length)

def format_number(number):
    """Format number with thousand separators (숫자를 천 단위 콤마로 포맷)"""
    return f"{number:,}"

def format_percent(value, total):
    """Calculate and format percentage (백분율 계산 및 포맷)"""
    if total == 0:
        return "0%"
...
def is_valid_email(email):
    """Simple email validation (이메일 유효성 검사, 간단)"""
...
```python
# main.py - Using utility module (유틸리티 모듈 사용)
import utils

utils.print_title("Grade Statistics (성적 통계)")

scores = [85, 90, 78, 92, 88]
total = sum(scores)  # Total score (총점)

print(f"\nTotal (총점): {utils.format_number(total)} points")
print(f"Average (평균): {total/len(scores):.1f} points")

# Count students who scored 80 or above (80점 이상 학생 수)
pass_count = sum(1 for s in scores if s >= 80)
print(f"80+ score rate (80점 이상): {utils.format_percent(pass_count, len(scores))}")
...
## 5️⃣ The Magic of `__name__` (__name__ 변수의 마법)

A Python file can be executed in two ways:

파이썬 파일은 두 가지 방식으로 실행될 수 있습니다:

1. **Direct execution (직접 실행)**: Running `python myfile.py` from the terminal
2. **Import execution (import로 실행)**: When another file does `import myfile`

Use the `__name__` variable to distinguish between these two situations.

이 두 가지 상황을 구분하고 싶을 때 `__name__` 변수를 사용합니다.

### What is the `__name__` Variable? (__name__ 변수란?)

Every Python file automatically has a special variable called `__name__`. Its value is:

모든 파이썬 파일에는 자동으로 `__name__`이라는 특별한 변수가 있습니다. 이 변수의 값은:

- When the file is **run directly**: `"__main__"`
- When the file is **imported**: the file name (module name)

### Why Is This Useful? (왜 이게 필요할까요?)

When building a module, you want to test whether functions work correctly. But when the module is imported by another file, you don't want the test code to run. This is exactly where `__name__` comes in!
...
```python
# calculator.py - Module with self-test (자체 테스트 기능이 있는 모듈)
def add(a, b):
    """Add two numbers (두 수를 더합니다)"""
    return a + b
...
# (이 파일을 직접 실행할 때만 실행됩니다)
if __name__ == "__main__":
    print("=" * 40)
    print("Calculator Module Test (계산기 모듈 테스트)")
    print("=" * 40)
...
**Situation 1: Running calculator.py directly (직접 실행할 때)**

```
$ python calculator.py

========================================
Calculator Module Test (계산기 모듈 테스트)
========================================
10 + 5 = 15
10 - 5 = 5
...
Because `__name__` equals `"__main__"`, the code inside the `if` block runs.

`__name__`이 `"__main__"`이므로 if 문 안의 코드가 실행됩니다.

**Situation 2: Importing from another file (다른 파일에서 import할 때)**

**main.py:**

```python
# main.py - Importing calculator module (calculator 모듈 import)
import calculator  # Import here (여기서 import)

result = calculator.add(20, 30)
print(f"Result (결과): {result}")  # Result: 50
```
...
This time, `__name__` is `"calculator"` (the module name), so the test code inside the `if` block does not run.

이번에는 `__name__`이 `"calculator"`(모듈 이름)이므로 if 문 안의 테스트 코드가 실행되지 않습니다!

### Analogy (이해를 돕는 비유)

`if __name__ == "__main__"` is like saying:

"If I'm the main character (main), run this code. But if I'm a supporting role (imported module), don't run it."

`if __name__ == "__main__"`은 마치 이렇게 말하는 것과 같습니다: "내가 주인공(main)이면 이 코드를 실행해. 하지만 내가 조연(import된 모듈)이면 실행하지 마."

### Example 4: Module with Testable Functions (예제 4: 테스트 가능한 모듈)

A module with built-in self-testing functionality.
...
```python
# string_utils.py - String utility module (문자열 유틸리티 모듈)
"""
String utility module (문자열 유틸리티 모듈)
"""
...
def count_vowels(text):
    """Count vowels in a string (모음 개수 세기)"""
    vowels = "aeiouAEIOU"
    count = 0
    for char in text:
        if char in vowels:
...
def remove_spaces(text):
    """Remove all spaces (공백 제거)"""
    return text.replace(" ", "")

def capitalize_words(text):
    """Capitalize the first letter of each word (각 단어의 첫 글자를 대문자로)"""
    words = text.split()
    result = []
    for word in words:
        if len(word) > 0:
...
# Module self-test (모듈 테스트)
if __name__ == "__main__":
    print("=" * 40)
    print("String Utility Test (문자열 유틸리티 테스트)")
    print("=" * 40)

    test_text = "Hello World"

    print(f"\nOriginal (원본): {test_text}")
    print(f"Reversed (뒤집기): {reverse_string(test_text)}")
    print(f"Vowel count (모음 개수): {count_vowels(test_text)}")
    print(f"No spaces (공백 제거): {remove_spaces(test_text)}")
    print(f"Capitalized (단어 대문자): {capitalize_words(test_text)}")
...
## 6️⃣ Packages — Collections of Modules (패키지 - 모듈의 모음)
...
## 7️⃣ Using External Packages (외부 패키지 사용하기)
...
## 8️⃣ Structuring a Real Project (실전 프로젝트 구성하기)

How should you organize files when building a real project? A small program may fit in one file, but even slightly larger programs benefit from a clear structure.

실제 프로젝트를 만들 때는 어떻게 파일을 구성해야 할까요? 작은 프로그램이라면 파일 하나로 충분하지만, 조금만 커져도 체계적인 구성이 필요합니다.

### Principles of Good Project Structure (좋은 프로젝트 구조의 원칙)

A good project structure separates code by function, uses a hierarchical folder layout from broad categories to specific ones, uses clear and descriptive names for files and folders, and keeps configuration code separate from logic code.

좋은 프로젝트 구조는 기능별로 코드를 분리하고, 큰 카테고리에서 작은 카테고리로 계층 구조를 이루며, 폴더와 파일 이름만 봐도 내용을 알 수 있게 명확히 짓고, 설정 관련 코드는 따로 관리합니다.

### Small Project Example (작은 프로젝트 예시)

```
simple_calculator/
│
├── main.py              # Program entry point (프로그램 시작점)
├── calculator.py        # Calculation functions (계산 함수들)
└── README.md            # Project description (프로젝트 설명 문서)
```

For a simple project, this structure is sufficient.

간단한 프로젝트는 이 정도면 충분합니다.

### Medium Project Example (중간 크기 프로젝트 예시)

```
shopping_mall/
│
├── main.py              # Program entry point (프로그램 시작점)
├── config.py            # Settings: DB address, API keys (설정)
├── requirements.txt     # Required external packages (필요한 외부 패키지 목록)
│
├── models/              # Data models (데이터 모델)
│   ├── __init__.py
│   ├── user.py          # User info (사용자 정보)
│   └── product.py       # Product info (상품 정보)
│
├── utils/               # Utility functions (유틸리티 함수들)
│   ├── __init__.py
│   ├── database.py      # Database-related (데이터베이스 관련)
│   └── helpers.py       # Miscellaneous helpers (기타 도우미 함수)
│
...
### Large Project Example (큰 프로젝트 예시)

```
game_project/
│
├── main.py              # Game entry point (게임 시작점)
├── config/              # Config package (설정 패키지)
│   ├── __init__.py
│   ├── settings.py      # Screen size, difficulty (게임 설정)
│   └── constants.py     # Colors, speeds (색상, 속도 등)
│
├── game/                # Game logic package (게임 로직 패키지)
│   ├── __init__.py
│   │
│   ├── player/          # Player-related (플레이어 관련)
│   │   ├── __init__.py
│   │   ├── character.py    # Character class (캐릭터 클래스)
│   │   ├── inventory.py    # Inventory (인벤토리)
│   │   └── skills.py       # Skill system (스킬 시스템)
│   │
│   ├── enemy/           # Enemy-related (적 관련)
│   │   ├── __init__.py
│   │   ├── monster.py      # Regular monsters (일반 몬스터)
│   │   └── boss.py         # Boss monsters (보스 몬스터)
│   │
│   └── items/           # Items (아이템 관련)
│       ├── __init__.py
│       ├── weapon.py       # Weapons (무기)
│       └── potion.py       # Potions (물약)
│
├── ui/                  # User interface (사용자 인터페이스)
│   ├── __init__.py
│   ├── menu.py          # Menu screen (메뉴 화면)
│   └── hud.py           # In-game UI (게임 중 UI)
│
├── utils/               # Utilities (유틸리티)
│   ├── __init__.py
│   ├── file_manager.py  # Save/load (파일 저장/불러오기)
│   └── calculator.py    # Calculation functions (계산 함수)
│
├── assets/              # Game resources (게임 리소스)
│   ├── images/
│   ├── sounds/
│   └── data/
│
├── tests/               # Test code (테스트 코드)
│   ├── test_player.py
│   └── test_enemy.py
│
├── requirements.txt     # Required packages list (필요한 패키지 목록)
└── README.md            # Project description (프로젝트 설명)
```

### requirements.txt File (requirements.txt 파일)

`requirements.txt` stores the list of external packages needed by a project. When someone receives your project, they can install all required packages at once using this file.

`requirements.txt`는 프로젝트에 필요한 외부 패키지 목록을 저장하는 파일입니다. 다른 사람이 우리 프로젝트를 받았을 때, 이 파일만 보고 필요한 패키지를 한 번에 설치할 수 있습니다.

**requirements.txt example (예시):**
...
**Install all at once (패키지 한 번에 설치):**

```bash
pip install -r requirements.txt
```

**Save currently installed packages (현재 설치된 패키지 목록 저장):**

```bash
pip freeze > requirements.txt
```

### Project Structure Tips (프로젝트 구성 팁)

**1. Clear Separation (명확한 분리)**

```python
# Bad: Everything in one file (나쁜 예: 모든 것을 한 파일에)
# game.py (3000 lines)
def create_player():
    pass
def create_enemy():
...
# Good: Separate by function (좋은 예: 기능별로 분리)
# player.py
def create_player():
    pass
...
**2. Using `__init__.py` (__init__.py 활용)**

```python
# game/__init__.py - Package initialization (패키지 초기화)
"""
Game main package (게임 메인 패키지)
"""

# Import frequently used items here (자주 사용하는 것들을 여기서 import)
from .player.character import Player
from .enemy.monster import Monster

# Now from other places you can use: (이러면 다른 곳에서 이렇게 쓸 수 있음)
# from game import Player, Monster
```

**3. Relative Imports (상대 경로 import)**

```python
# game/player/character.py
from ..items import Weapon    # items package in parent folder (상위 폴더의 items 패키지)
from .inventory import Inventory  # inventory module in same folder (같은 폴더의 inventory 모듈)
```

### How to Start a Project (프로젝트 시작 방법)

Start with 1–2 files and keep things simple. When a file exceeds 100 lines, split it by function. Group related files into folders as the project grows. Add tests for important features as you go.

처음에는 1~2개 파일로 간단하게 시작합니다. 파일이 100줄을 넘으면 기능별로 분리합니다. 관련 파일들을 폴더로 묶어가며 점진적으로 확장합니다. 중요한 기능에는 테스트 코드를 추가합니다.

Growing the project gradually like this makes it much easier to manage later!

이렇게 점진적으로 프로젝트를 키워가면, 나중에 관리하기 훨씬 쉽습니다!

### Example 5: Simple Game System (예제 5: 간단한 게임 시스템)

**game/player.py:**

```python
# player.py - Player management module (플레이어 관리 모듈)
"""
Player management module (플레이어 관리 모듈)
"""

class Player:
    def __init__(self, name):
        # Initialize player attributes (플레이어 속성 초기화)
        self.name = name
        self.health = 100
        self.level = 1
        self.experience = 0
    ...
    def gain_exp(self, exp):
        """Gain experience points (경험치 획득)"""
        self.experience = self.experience + exp

        # Check for level up (레벨업 체크)
    ...
    def level_up(self):
        ...
        self.health = 100
        print(f"\n🎉 Level Up! (레벨업!) Lv.{self.level}")

    def show_status(self):
        """Display status (상태 표시)"""
        print(f"\n[ {self.name} ]")
        print(f"Level (레벨): {self.level}")
        print(f"Health (체력): {self.health}/100")
        print(f"EXP (경험치): {self.experience}/{self.level * 100}")
...
if __name__ == "__main__":
    ...
    player.take_damage(30)  # Take 30 damage (데미지 30 받음)
    print("\nTook 30 damage (데미지 30 받음)")
    player.show_status()

    player.heal(20)  # Heal 20 (회복 20)
    print("\nHealed 20 (회복 20)")
    player.show_status()

    player.gain_exp(150)  # Gained 150 EXP (경험치 150 획득)
    print("\nGained 150 EXP (경험치 150 획득)")
    player.show_status()
```
...
```python
# main.py - RPG Game using player module (player 모듈을 사용하는 RPG 게임)
from game.player import Player
import random

print("⚔️" + "=" * 38 + "⚔️")
print("   RPG Game (RPG 게임)")
print("⚔️" + "=" * 38 + "⚔️")
...
while True:
    ...
    if choice == "1":
        damage = random.randint(10, 30)  # Random damage (랜덤 데미지)
        exp = random.randint(20, 50)      # Random EXP (랜덤 경험치)

        print(f"\n⚔️ Monster Battle! (몬스터와 전투!)")
        print(f"Damage (데미지): {damage}")
        print(f"EXP (경험치): +{exp}")
        ...
        if player.health == 0:
            print("\n💀 Health reached 0! (체력이 0이 되었습니다!)")
            print("Game Over (게임 오버)")
            break

    elif choice == "2":
        heal_amount = random.randint(20, 40)  # Random heal amount (랜덤 회복량)
        print(f"\n💤 Rested and recovered {heal_amount} HP (휴식하여 {heal_amount} 회복)")
