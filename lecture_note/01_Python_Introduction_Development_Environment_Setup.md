...
Identify all the problems in the following code:

다음 코드에서 문제점을 모두 고르시오.
...
1. Print should be written in lowercase as print
2. String quote is not closed
3. The result of input() is not converted with int(), so numeric calculation is not possible
4. All correct
...
All problems exist. (1) `Print` should be written as `print`, (2) the closing quote for the string is missing, and (3) if you don't convert the string received by `input()` with `int()`, `number * 2` becomes a string repetition result (for example, "5" * 2 = "55").
