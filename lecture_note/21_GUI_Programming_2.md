...
## 7️⃣ Practical Example: Survey Program (실전 예제: 설문조사 프로그램)

A survey program using the widgets learned so far.
...
{'='*30}
    ...
    messagebox.showinfo("Submission Complete", result)
...
# 1. Name (이름)
tk.Label(window, text="Name:", bg="white", font=("Arial", 11)).pack(anchor="w", padx=30)
name_entry = tk.Entry(window, width=30, font=("Arial", 11))
name_entry.pack(anchor="w", padx=30, pady=5)

# 2. Gender (Radio Button) (성별)
tk.Label(window, text="Gender:", bg="white", font=("Arial", 11)).pack(anchor="w", padx=30, pady=(15, 5))
gender_var = tk.StringVar(value="Male")

gender_frame = tk.Frame(window, bg="white")  # Gender frame (성별 프레임)
...
# 3. Age (Slider) (나이)
tk.Label(window, text="Age:", bg="white", font=("Arial", 11)).pack(anchor="w", padx=30, pady=(15, 5))
age_scale = tk.Scale(
    window,
    from_=10,
    to=80,
...
# 4. Hobbies (Checkbox) (취미)
tk.Label(window, text="Hobbies (multiple selection):", bg="white", font=("Arial", 11)).pack(anchor="w", padx=30, pady=(15, 5))

hobby1_var = tk.IntVar()  # Sports (운동)
hobby2_var = tk.IntVar()  # Reading (독서)
hobby3_var = tk.IntVar()  # Movie (영화)

hobby_frame = tk.Frame(window, bg="white")  # Hobby frame (취미 프레임)
hobby_frame.pack(anchor="w", padx=30)
tk.Checkbutton(hobby_frame, text="Sports", variable=hobby1_var, bg="white").pack(anchor="w")
tk.Checkbutton(hobby_frame, text="Reading", variable=hobby2_var, bg="white").pack(anchor="w")
tk.Checkbutton(hobby_frame, text="Movie", variable=hobby3_var, bg="white").pack(anchor="w")

# 5. Opinion (Text) (의견)
tk.Label(window, text="Opinion:", bg="white", font=("Arial", 11)).pack(anchor="w", padx=30, pady=(15, 5))

text_frame = tk.Frame(window)  # Text frame (의견 입력)
text_frame.pack(padx=30, pady=5)

scrollbar = tk.Scrollbar(text_frame)  # Scrollbar (스크롤바)
scrollbar.pack(side="right", fill="y")

opinion_text = tk.Text(text_frame, width=40, height=5  # Opinion text (의견 텍스트), yscrollcommand=scrollbar.set)
opinion_text.pack(side="left")
scrollbar.config(command=opinion_text.yview)  # Connect scrollbar (스크롤바 연결)
...
## 8️⃣ Practical Example: Simple Calculator (Final) (실전 예제: 간단한 계산기 - 완성판)
