...
1. **User Input**: Users can enter unexpected values
   **사용자 입력**: 사용자는 예상 못한 값을 입력할 수 있음
...
### What You Can Do with Exception Info (예외 정보로 할 수 있는 일들)

**1. Provide detailed information to users**
...
try:
    ...
    # Show user-friendly message (사용자 친화적 메시지 표시)
...
try:
    ...
    # Log the error to file (오류를 파일에 기록)
    with open("error_log.txt", "a") as log:
        timestamp = datetime.datetime.now()
        log.write(f"[{timestamp}] {type(e).__name__}: {e}\n")
  
    # Show simple message to user (사용자에게는 간단한 메시지)
...
try:
    ...
    # Get exception type name and message (예외 타입 이름과 메시지 구하기)
...
### Practical Example: Dictionary Search (실전 예제: 딕셔너리 검색)

```python
# Fruit price search program with improved error handling (개선된 오류 처리 - 과일 가격표)
fruits = {
    "apple": 3000,
    "banana": 2000,
    "orange": 4000
...
try:
    # Try to find the fruit (과일 찾기 시도)
    price = fruits[fruit]
    print(f"✓ {fruit}: {price:,} won")
  
except KeyError as e:
    # Fruit not in dictionary (등록되지 않은 과일)
    print(f"❌ Fruit not found!")
    print(f"💡 Looking for: {e}")
  
    # Suggest similar names (비슷한 이름 추천)
...
## 6️⃣ else - Runs Only When No Exception (else - 예외가 없을 때만 실행)

The `else` block runs **only when no exception occurred** in the try block.

`else` 블록은 `try` 블록에서 **예외가 발생하지 않았을 때만** 실행됩니다.

### Why Use else? (왜 else를 사용할까요?)

Use it when you have code that should only run if no exceptions occurred. It keeps your try block focused on just the risky code.

"예외가 없을 때만 실행해야 하는 코드"가 있을 때 사용합니다. try 블록 안에 너무 많은 코드를 넣는 것보다, 정말 예외가 발생할 수 있는 부분만 try에 넣는 것이 더 명확합니다.
...
try:
    ...
    # Only runs if no exception occurred (예외가 없었을 때만 실행)
...
### Practical Example: Complete File Processing (실전 예제: 파일 처리 완전판)

```python
# Complete file processing with all blocks (모든 블록을 포함한 완전한 파일 처리)
filename = input("Filename: ")

try:
    # Open file (파일 열기 시도 - 예외 발생 가능)
    with open(filename, "r", encoding="utf-8") as file:
        content = file.read()
    ...
    # No exception occurred (예외가 없었을 때)
    print("\n" + "=" * 40)
    print(f"📖 {filename}")
    print("=" * 40)
    print(content)
    ...
    # Always runs (항상 실행 - 정리 작업)
    print("File processing complete")
```

---

## 7️⃣ Raising Exceptions (예외 직접 발생시키기 - raise)
...
### Practical Example: Bank Account Class (실전 예제: 은행 계좌 클래스)

```python
# Bank account with validation (검증이 있는 은행 계좌)
class BankAccount:
    """Simple bank account (간단한 은행 계좌)"""
  
    def __init__(self, owner, initial_balance=0):
        """Initialize account (계좌 초기화)"""
        self.owner = owner
...
try:
    ...
    account.withdraw(20000)  # This will fail! (실패할 것!)
...
### Creating Custom Exceptions (사용자 정의 예외 만들기)

```python
# Define your own exception (사용자 정의 예외)
class InvalidAgeError(Exception):
    """Raised when age is invalid (나이가 유효하지 않을 때 발생)"""
    pass

class InvalidPasswordError(Exception):
    """Raised when password is invalid (비밀번호가 유효하지 않을 때 발생)"""
    pass
...
## 8️⃣ Practical Applications (실전 활용)

### Example 7: Safe ATM (예제 7: 안전한 ATM)
...
- Handle exceptions safely with try-except
  try-except로 예외를 안전하게 처리하세요
- Specify exact exception types (ValueError, KeyError, etc.)
  구체적인 예외 타입을 지정하세요 (ValueError, KeyError 등)
- Use `as` to get exception messages
  `as`를 사용하여 예외 메시지를 받으세요
- `finally` always runs - use for cleanup
  `finally`는 항상 실행됩니다 - 정리 작업에 사용하세요
