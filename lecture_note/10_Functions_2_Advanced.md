...
### Example 1: Statistics Calculator (예제 1: 통계 계산)

A function that calculates sum, average, maximum, and minimum of multiple numbers at once.

여러 숫자의 합계, 평균, 최댓값, 최솟값을 한 번에 계산하는 함수입니다.

```python
# Statistics calculation function (통계 계산 함수)
...
print("📊" + "=" * 38 + "📊")
print("   Score Statistics Analysis")
print("📊" + "=" * 38 + "📊")
...
print(f"Highest: {max_val}")
print(f"Lowest: {min_val}")
print("=" * 40)
```
...
### Example 2: Game Score Management (예제 2: 게임 점수 관리)
...
### Example 3: Data Processing (예제 3: 데이터 처리)
...
### Example 4: Fibonacci Sequence (예제 4: 피보나치 수열)
...
### Example 5: Sum of List (예제 5: 리스트의 합)
...
```python
# Sum list elements using recursion (재귀로 리스트 합 계산)
def sum_list(numbers):
    # Base case: empty list (기저 사례: 빈 리스트)
    if len(numbers) == 0:
        return 0
    
    # Recursive case: first + sum of rest (재귀 사례: 첫 요소 + 나머지의 합)
...
### Example 6: Flexible Calculator (예제 6: 유연한 계산기)
