...
## 6️⃣ Practical Example: Sales Analysis Program (실전 예제: 매출 분석 프로그램)

A simple program that analyzes sales data.

간단한 매출 데이터를 분석하는 프로그램입니다.

### Data Preparation (데이터 준비)

First, let's create a sales data file.

먼저 매출 데이터 파일을 만들어봅시다.

**Creating sales.csv file (sales.csv 파일 만들기):**

**sales.csv file contents (파일 내용):**

```
Date,Product,Quantity,Price
2024-01-01,Laptop,2,1500000
2024-01-01,Mouse,5,30000
2024-01-02,Keyboard,3,120000
2024-01-02,Monitor,1,350000
2024-01-03,Laptop,1,1500000
2024-01-03,Mouse,3,30000
```

### Sales Analysis Code (매출 분석 코드)
...
for sale in sales:
    ...
    if date not in dates:
        dates[date] = 0
    dates[date] += sale['Revenue']

for date in sorted(dates.keys()):
    print(f"{date}: {dates[date]:,} won")

# 2. Product sales analysis (상품별 매출 분석)
print("\n" + "=" * 60)
...
for sale in sales:
    ...
    if product not in products:
        products[product] = 0
    products[product] += sale['Revenue']

for product in sorted(products.items(), key=lambda x: x[1], reverse=True):
    print(f"{product[0]}: {product[1]:,} won")

# 3. Save results to CSV (결과를 CSV로 저장)
with open('sales_summary.csv', 'w', newline='', encoding='utf-8') as file:
    fieldnames = ['Category', 'Item', 'Sales']
    csv_writer = csv.DictWriter(file, fieldnames=fieldnames)
    
    csv_writer.writeheader()
    
    for date, revenue in sorted(dates.items()):
        csv_writer.writerow({
            'Category': 'By Date',
            'Item': date,
            'Sales': revenue
...
## 7️⃣ Practical Example: Survey Result Analysis (실전 예제: 설문조사 결과 분석)

A program that analyzes survey data.
...
John Smith,25,Male,5
Sarah Davis,30,Female,4
Michael Johnson,28,Male,5
Emily Wilson,22,Female,3
James Brown,35,Male,4
Jessica Martinez,27,Female,5
```

### Survey Analysis Code (설문조사 분석 코드)
...
print("=" * 60)
print("📊 Survey Result Analysis")
print("=" * 60)

# 1. Basic statistics (기본 통계)
print(f"\nNumber of Respondents: {len(responses)} people")

ages = [r['Age'] for r in responses]
print(f"Average Age: {sum(ages) / len(ages):.1f} years old")
print(f"Minimum Age: {min(ages)} years old")
print(f"Maximum Age: {max(ages)} years old")

# 2. Gender distribution (성별 분포)
gender_count = Counter(r['Gender'] for r in responses)
print("\n👥 Gender Distribution:")
for gender, count in gender_count.items():
    percentage = (count / len(responses)) * 100
    print(f"   {gender}: {count} people ({percentage:.1f}%)")

# 3. Satisfaction analysis (만족도 분석)
...
for score in sorted(satisfaction_count.keys(), reverse=True):
    ...
    bar = '■' * count
    print(f"   {score} points: {bar} ({count} people)")

# 4. Gender satisfaction comparison (성별 만족도 비교)
print("\nGender Satisfaction Comparison:")
male_satisfaction = [r['Satisfaction'] for r in responses if r['Gender'] == 'Male']
female_satisfaction = [r['Satisfaction'] for r in responses if r['Gender'] == 'Female']

if male_satisfaction:
    print(f"   Male average: {sum(male_satisfaction) / len(male_satisfaction):.2f}")
if female_satisfaction:
    print(f"   Female average: {sum(female_satisfaction) / len(female_satisfaction):.2f}")

# 5. Age group analysis (연령대별 분석)
print("\n📈 Age Group Satisfaction:")
age_groups = {'20s': [], '30s': []}

for r in responses:
    if 20 <= r['Age'] < 30:
        age_groups['20s'].append(r['Satisfaction'])
    elif 30 <= r['Age'] < 40:
        age_groups['30s'].append(r['Satisfaction'])

for age_group, satisfactions in age_groups.items():
    if satisfactions:
        avg = sum(satisfactions) / len(satisfactions)
        print(f"   {age_group}: {avg:.2f} ({len(satisfactions)} people)")
...
## 8️⃣ Data Filtering and Sorting (데이터 필터링과 정렬)

Methods to filter and sort CSV data according to conditions.

CSV 데이터를 조건에 맞게 필터링하고 정렬하는 방법입니다.

### Filtering Example (필터링 예제)
...
# Calculate total score (총점 계산)
for student in students:
    student['Total'] = student['Korean'] + student['English'] + student['Math']
    student['Average'] = student['Total'] / 3

# 1. Find students with average 90 or higher (평균 90점 이상 우수 학생)
print("🏆 Excellent Students (Average 90 or Higher):")
excellent = [s for s in students if s['Average'] >= 90]
for student in excellent:
    print(f"   {student['Name']}: {student['Average']:.1f} points")

# 2. Find students with 80 or higher in Math (수학 80점 이상 학생)
print("\n📐 Students with Math 80 or Higher:")
good_math = [s for s in students if s['Math'] >= 80]
for student in good_math:
    print(f"   {student['Name']}: {student['Math']} points")

# 3. Save filtered data to new CSV (필터링된 데이터를 새 CSV로 저장)
with open('excellent_students.csv', 'w', newline='', encoding='utf-8') as file:
    fieldnames = ['Name', 'Korean', 'English', 'Math', 'Total', 'Average']
    csv_writer = csv.DictWriter(file, fieldnames=fieldnames)
  
    csv_writer.writeheader()
    csv_writer.writerows(excellent)

print("\n✓ Excellent student list has been saved to excellent_students.csv.")
```

---

## 9️⃣ Tips for Handling CSV Files (CSV 파일 다루기 팁)

### Tip 1: Check if file exists (파일 존재 확인)
...
### Tip 2: Skip blank lines (빈 줄 건너뛰기)
...
with open('data.csv', 'r', encoding='utf-8') as file:
    csv_reader = csv.reader(file)
    for row in csv_reader:
        if not row:  # If blank line (빈 줄이면)
            continue  # Skip (건너뛰기)
        print(row)
```

### Tip 3: Append mode (데이터 추가 모드)

```python
import csv

# Add data to existing file (기존 파일에 데이터 추가)
with open('students.csv', 'a', newline='', encoding='utf-8') as file:
    csv_writer = csv.writer(file)
    csv_writer.writerow(['Robert Taylor', 24, 4])  # Add new data (새 데이터 추가)

print("✓ Data addition complete!")
```

### Tip 4: Use different delimiter (다른 구분자 사용)

When reading TSV (Tab-separated) instead of CSV:
...
# Read tab-separated file (탭으로 구분된 파일)
