...
A list is an ordered collection that stores multiple values. You can access elements by index, add or remove elements using methods like append(), insert(), remove(), and pop(). Slicing extracts portions of lists using the syntax list[start🔚step].

리스트는 순서가 있는 여러 값의 모음입니다. 인덱스로 요소에 접근하고, append(), insert(), remove(), pop() 등의 메서드로 추가하거나 제거할 수 있습니다. 슬라이싱은 list[start🔚step] 문법으로 리스트의 일부를 추출합니다.
...
In the next chapter, we will learn about tuples and sets. Tuples allow you to safely store immutable data, and sets enable you to efficiently manage data without duplicates!

다음 장에서는 튜플과 세트에 대해 배웁니다. 튜플은 변경할 수 없는 데이터를 안전하게 저장하고, 세트는 중복 없는 데이터를 효율적으로 관리할 수 있게 해줍니다.
