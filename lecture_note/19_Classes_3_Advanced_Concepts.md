...
### Practical Example: Compare Product Prices (실전 예제: 상품 가격 비교)
...
class Product:
    ...
    def __gt__(self, other):
        """Is more expensive? (가격이 더 비싼지)"""
        return self.price > other.price
  
    def __str__(self):
        return f"{self.name} ({self.price:,} won, ⭐{self.rating})"

# Product list (상품 목록)
products = [
    Product("Wireless Mouse", 35000, 4.5),
    Product("Mechanical Keyboard", 120000, 4.8),
    Product("Webcam", 80000, 4.2),
    Product("Monitor", 350000, 4.7)
]
...
# Cheapest and most expensive (가장 싼 상품과 비싼 상품)
...
### Practical Example: Time Class (실전 예제: 시간 클래스)

```python
class Time:
    """Time class (hours:minutes) (시간 클래스 - 시:분)"""
  
    def __init__(self, hour, minute):
        self.hour = hour
        self.minute = minute
        self._normalize()  # Convert if over 60 minutes (60분 넘으면 시간으로 변환)
  
    def _normalize(self):
        """Normalize time (시간 정규화)"""
        if self.minute >= 60:
            self.hour += self.minute // 60
            self.minute = self.minute % 60
        self.hour = self.hour % 24  # Reset if over 24 hours (24시간 넘으면 0으로)
    ...
    def __sub__(self, other):
        ...
        if diff < 0:
            diff += 24 * 60  # Next day if negative (음수면 다음날로)
...
work_time = Time(8, 30)      # 8 hours 30 minutes (8시간 30분 근무)
lunch_time = Time(1, 0)      # 1 hour lunch (1시간 점심)

work_end = work_start + work_time
print(f"Start work: {work_start}")
print(f"Work time: {work_time}")
print(f"End work: {work_end}")
...
### Practical Example: Student Grades (간단한 실전 예제: 학생 성적)
...
class Student:
    ...
    def score(self, value):
        """Write score (0-100 only) (점수 쓰기 - 0-100 사이만 허용)"""
        if value < 0:
    ...
    @property
    def grade(self):
        """Grade (read-only, auto-calculated) (학점 - 읽기 전용, 자동 계산)"""
        if self.score >= 90:
            return "A"
        elif self.score >= 80:
            return "B"
    ...
    def show_info(self):
        print(f"{self.name}: {self.score} points ({self.grade} grade)")

# Create student (학생 생성)
student = Student("Chulsu", 85)
student.show_info()  # Chulsu: 85 points (B grade)

# Change score (점수 변경)
student.score = 95
student.show_info()  # Chulsu: 95 points (A grade)

# Invalid score input (잘못된 점수 입력)
student.score = 150  # ❌ Score cannot exceed 100!
student.show_info()  # Chulsu: 95 points (A grade) - not changed!
```

**Summary (정리):**

- Property = Safe variable (프로퍼티 = 안전한 변수)
- `@property`: Read function (읽기 함수)
- `@변수이름.setter`: Write function with validation (쓰기 함수 - 검사 가능)
- Auto-calculated values should be read-only (자동 계산 값은 읽기 전용으로 만들기)
...
### Simple Example 2: Math Tools (간단한 예제 2: 계산 도구)

```python
class MathHelper:
    """Math toolkit (수학 도구 모음)"""
    ...
    def max_min_diff(numbers):
        """Difference between max and min (최댓값과 최솟값의 차이)"""
        if len(numbers) == 0:
            return 0
...
print(f"Average: {MathHelper.average(scores)}")           # 86.6
print(f"Score difference: {MathHelper.max_min_diff(scores)}")  # 14
print(f"Even count: {MathHelper.even_count(scores)}")    # 4
```

### When to Use? (언제 사용하나요?)

**Use Static Methods (정적 메서드 사용):**

- ✅ Calculator functions (addition, subtraction, etc.) (계산기 함수 - 덧셈, 뺄셈 등)
- ✅ String processing functions (문자열 처리 함수)
- ✅ Date calculation functions (날짜 계산 함수)
- ✅ Validation functions (유효성 검사 함수)

**Use Regular Methods (일반 메서드 사용):**

- ✅ When object data is needed (객체의 데이터가 필요할 때)
- ✅ When using self.name, self.age, etc. (self.name, self.age 등을 사용할 때)

### When to Use Class Methods? (클래스 메서드는 언제 쓰나요?)

Use when dealing with class variables (shared by all objects).

클래스 변수(모든 객체가 공유하는 변수)를 다룰 때 사용합니다.

```python
class Student:
    # Class variable (shared by all students) (클래스 변수 - 모든 학생이 공유)
    school_name = "Yeungnam University College"
    student_count = 0
  
    def __init__(self, name):
        self.name = name
        Student.student_count += 1  # Increment student count (학생 수 증가)
  
    @classmethod
    def get_school_name(cls):
        """Get school name (class variable) (학교 이름 - 클래스 변수)"""
        return cls.school_name
  
    @classmethod
    def get_student_count(cls):
        """Get total student count (총 학생 수)"""
        return cls.student_count
  
    @classmethod
    def set_school_name(cls, name):
        """Change school name (class variable) (학교 이름 변경 - 클래스 변수)"""
        cls.school_name = name

# Create 3 students (학생 3명 생성)
s1 = Student("Chulsu")
s2 = Student("Younghee")
s3 = Student("Minsu")

# Check all information with class method (클래스 메서드로 전체 정보 확인)
print(f"School: {Student.get_school_name()}")
print(f"Total students: {Student.get_student_count()}")

# Change school name (applied to all students) (학교 이름 변경 - 모든 학생에게 적용)
Student.set_school_name("Another University")
print(f"New school: {Student.get_school_name()}")
```

**Summary (정리):**

| Type | Decorator | First Parameter | When to Use? |
| --- | --- | --- | --- |
| Regular Method | None | self | Object data needed |
| 일반 메서드 | 없음 | self | 객체 데이터 필요 |
| Static Method | @staticmethod | None | Independent function |
| 정적 메서드 | @staticmethod | 없음 | 독립 함수처럼 |
| Class Method | @classmethod | cls | Access class variable |
| 클래스 메서드 | @classmethod | cls | 클래스 변수 접근 |

**Easy to Remember (간단히 기억하기):**

- **Regular Method**: "I need my (object's) data!" (일반 메서드: "나(객체)의 데이터 필요해!")
- **Static Method**: "I can work well on my own!" (정적 메서드: "나는 혼자서도 일 잘해!")
- **Class Method**: "I need our (class's) data!" (클래스 메서드: "우리(클래스) 모두의 데이터 필요해!")
